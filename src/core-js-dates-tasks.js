...
function getTime(date) {
  const two = (num) => num.toString(10).padStart(2, '0');
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.floor((date2 - date1) / (1000 * 3600 * 24)) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  const timeStart = Date.parse(period.start);
  const timeEnd = Date.parse(period.end);
  return timeStart <= timeEnd && time >= timeStart && time <= timeEnd;
}
...
function getWeekNumberByDate(date) {
  const date0 = new Date(date.getFullYear(), 0);
  const day0 = date0.getDay();
  let deltaDay;
  switch (day0) {
    case 1:
      deltaDay = 0;
  ...
  const date1 = new Date(date.getFullYear(), 0, 1 + deltaDay);
  return (
    (deltaDay ? 1 : 0) + Math.floor((date - date1) / (1000 * 3600 * 24) / 7) + 1
  );
}
...
function getQuarter(date) {
  return Math.ceil(date.getUTCMonth() / 3 + 0.1);
}
